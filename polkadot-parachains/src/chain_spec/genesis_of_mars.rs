use super::*;
use mars_runtime;
use mars_runtime::{AccountId, AuraId, Signature};
// use mars_runtime::{
//     CouncilConfig,
...
use odyssey_runtime::Balance as MarsBalance;

const AMAS_ED: MarsBalance = mars_runtime::constants::currency::EXISTENTIAL_DEPOSIT;

pub fn mars_session_keys(keys: AuraId) -> mars_runtime::opaque::SessionKeys {
    mars_runtime::opaque::SessionKeys { aura: keys }
}

pub(crate) fn mars_genesis(
    candidates: Vec<(AccountId, NimbusId, MarsBalance)>,
    nominations: Vec<(AccountId, AccountId, MarsBalance)>,
    root_key: AccountId,
    invulnerables: Vec<(AccountId, AuraId)>,
    // initial_authorities: Vec<AuraId>,
    endowed_accounts: Vec<AccountId>,
    id: ParaId,
) -> mars_runtime::GenesisConfig {
    ...
    mars_runtime::GenesisConfig {
        system: mars_runtime::SystemConfig {
            code: mars_runtime::WASM_BINARY
                .expect("WASM binary was not build, please build it!")
                .to_vec(),
            changes_trie_config: Default::default(),
        },
        balances: mars_runtime::BalancesConfig {
        ...
        sudo: mars_runtime::SudoConfig { key: root_key.clone() },
        parachain_info: mars_runtime::ParachainInfoConfig { parachain_id: id },
        collator_selection: mars_runtime::CollatorSelectionConfig {
            invulnerables: invulnerables.iter().cloned().map(|(acc, _)| acc).collect(),
            candidacy_bond: AMAS_ED,
            ..Default::default()
        },
        session: mars_runtime::SessionConfig {
            keys: invulnerables
                .iter()
                .cloned()
                .map(|(acc, aura)| {
                    (
                        ...
                        mars_session_keys(aura), // session keys
                    )
                })
                .collect(),
        },
        democracy: mars_runtime::DemocracyConfig::default(),
        council: mars_runtime::CouncilConfig::default(),
        technical_committee: mars_runtime::TechnicalCommitteeConfig {
        ...
        vesting: mars_runtime::VestingConfig { vesting: vec![] },
        ...
        // },
        ocw_module: mars_runtime::OCWModuleConfig {
